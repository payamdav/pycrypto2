...
def build_figure(res: dict, cfg: dict):
    """Full chart (candles, vwap, domes, POC lines) + POC table. No file I/O."""
    import plotly.graph_objects as go
    from plotly.subplots import make_subplots

    data, vwap, domes = res["data"], res["vwap"], res["domes"]
...
def analyze(cfg: dict) -> dict:
    """Load candles, scan for domes, dedupe, compute the volume profile +
    ...
    return {
        ...
        "domes": domes, "vp": vp_res["vp"], "pocs": vp_res["pocs"],
    }
...
def run(cfg: dict) -> None:
    ...
    out_path = SCRIPT_DIR / f"falling_from_dome_{cfg['asset']}.html"
